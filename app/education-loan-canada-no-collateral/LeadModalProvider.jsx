 ...
 *   window.dispatchEvent(new CustomEvent("open-lead-modal", { detail: { title: "Check Loan Eligibility" } }));
 * If no title is passed, defaults to "Check Loan Eligibility".
 */
export default function LeadModalProvider() {
  ...
  const onChange = (e) => {
    ...
    try {
      const res = await fetch("/api/leads-canada", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          intakeMonth,
          intakeYear,
          // utm: window?.__SS_UTM || undefined, // if you capture UTMs later
        }),
      });
        ...
        {!done ? (
          ...
          <div className={styles.modalBody}>
            <div className={styles.thanks}>
              ...
              <button className={styles.secondaryCta} onClick={() => setOpen(false)} type="button">
                Close
              </button>
            </div>
          </div>
